...
const AdminDocMonthly = () => {
  ...
  useEffect(()=>{
    const getStudents = async () => {
      try {
        const response = await axios.get(`${url}/api/admin/getMonthlyReportsAvailableStudents`);
  ...
  useEffect(() => {
    const groupByStudent = (data) => {
      ...
      data.forEach(item => {
        ...
        grouped[item.registrationNumber].reports.push(item.monthly);
      });
      return grouped;
    };
            ...
            {Object.keys(filteredData).map(studentId => {
              ...
              return (
                ...
                <React.Fragment key={studentId}>
                  ...
                  {isExpanded && reports.map((monthlyReports, outerIndex) =>
              ...
              {isExpanded && (
                <div className="border-t border-gray-100">
                  {reports.map((monthlyReports) => (
                    ...
                    >
                      <div>
                        <div className="font-medium text-sm">{report.month}</div>
...
export default AdminDocMonthly;