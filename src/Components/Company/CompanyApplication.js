...
const CompanyApplication = () => {
      ...
      const getApplicants = async () => {
        ...
        if (response.data.success) {
          ...
          const uniquePositions = [
            ...new Set(sortedApplicants.map((app) => app.position)),
          ];
      ...
      const analyzed = await Promise.all(
        positionApplicants.map(async (applicant) => {
          try {
            ...
            console.error(
              "Error analyzing CV for applicant:",
              applicant.userName,
              error
            );
            return {
  ...
  const handleHire = async (id) => {
    try {
      const response = await axios.put(`${url}/api/company/updateHire`, {
        id,
        isHired: true,
      });
      if (response.data.success) {
        setFilteredApplicants((prevApplicants) =>
          prevApplicants.map((student) =>
            student._id === id ? { ...student, isHired: true } : student
          )
        );
        setShortlistedCandidates((prevCandidates) =>
          prevCandidates.map((student) =>
      ...
      <div className="flex flex-col ml-4 mt-16">
        ...
        <div className="mb-6">
          <h3 className="text-lg font-medium mb-2">
            Filter by Position (Shortlist CVs)
          </h3>
        ...
        <div className="mx-4 mb-8">
          ...
          {isLoading ? (
                ...
                <tbody>
                  {filteredShortlisted.map((candidate, index) => (
                    <tr
                      ...
                      className={`border-b ${
                        candidate.matchScore >= 90 ? "bg-green-50" : ""
                      }`}
                    >
            ...
            <div className="bg-yellow-50 border border-yellow-100 rounded-md p-4 text-center">
              No candidates meet the current match threshold ({matchThreshold}
        ...
        <table className="w-full table-auto">
          <thead>
            <tr className="bg-gray-200">
              ...
              <th className="px-4 py-2 text-left">CV</th>
              <th className="px-4 py-2 text-left">Hire Status</th>
            </tr>
          </thead>
          <tbody>
                ...
                <tr key={index} className="border-b">
                  ...
                  <td className="px-4 py-2 flex flex-col">
                    {cvStatus === applicant.status ? (
                      ...
                      <span className="bg-green-500 text-white px-3 py-1 rounded-full text-sm text-center shadow-md">
                        Accepted
                      </span>
                    ) : (
                      <span className="bg-red-500 text-white px-3 py-1 rounded-full text-sm text-center shadow-md">
                        Rejected
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-2">
                    {applicant.status ? (
                      <>
                        {applicant.isHired ? (
                          <>
                            <span className="bg-blue-500 text-white px-3 py-1 rounded-full text-sm text-center shadow-md">
                              Hired
                            </span>
                          </>
                        ) : (
                          <>
                            <button
                              ...
                              className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm mb-1 shadow-lg"
                            >
                              Hire
                            </button>
                          </>
  ...
  <table className="w-full table-auto">
    ...
    <tbody>
      {filteredApplicants.length > 0 ? (
        filteredApplicants
          .filter((applicant) => {
            if (statusFilter === 'Accepted') return applicant.status === true;
            if (statusFilter === 'Rejected') return applicant.status === false;
            if (statusFilter === 'Hired') return applicant.isHired === true;
            return true; // for 'All'
          })
          .map((applicant, index) => (
            <tr key={index} className="border-b">
